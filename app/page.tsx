  ...
  return (
    <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
        ...
        <Card title="Keywords" value={loading ? '…' : keywords.length} sub="across all reports" />
        <Card title="Backlinks" value={loading ? '…' : backlinks.length} sub="across all reports" />
        <Card title="Competitor Pages" value={loading ? '…' : competitors.length} sub="indexed" />
        <Card title="Keyword Gaps" value={loading ? '…' : gaps.length} sub="gap opportunities" />
